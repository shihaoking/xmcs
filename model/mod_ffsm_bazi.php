...
/*
        $cookies = $row;
        $parem = http_build_query($cookies);
        $url = 'http://www.miquanapp.com/app/bzsm.php?'.$parem;
        $c = file_get_contents($url);
        $data = json_decode($c,true);

        $data['shiye'] = self::xishensy($data['data']['xishen']);
        $data['sanhe'] = self::sanhe($row['sx']);
        return $data;*/
    }
...
6、吉利楼层末位数为：0 5 <br>';
    ...
    function bazipp($row){
       ...
       $lnp=1;
		...
		//十二长生
			$srchangsheng[]=cls_paipan::getChangshengg($yg,$yzx);
			$srchangsheng[]=cls_paipan::getChangshengg($mg,$mzx);
			$srchangsheng[]=cls_paipan::getChangshengg($rg,$rzx);
			$srchangsheng[]=cls_paipan::getChangshengg($hg,$hzx);
		
			//神煞
			$shenshas_y[]=cls_paipan::Shen_rigan($yg,$yz);
			$shenshas_y[]=cls_paipan::Shen_rizhi($rz,$yz,$yzx);
			$shenshas_y[]=cls_paipan::Shen_yuezhi($mz,$yg,$yz);
			$shenshas_y[]=cls_paipan::Shen_nianzhi($yz,$yz,$yzx,1);
			
			$shenshas_m[]=cls_paipan::Shen_rigan($mg,$mz);
			$shenshas_m[]=cls_paipan::Shen_rizhi($rz,$mz,$mzx);
			$shenshas_m[]=cls_paipan::Shen_yuezhi($mz,$mg,$mz);
			$shenshas_m[]=cls_paipan::Shen_nianzhi($yz,$mz,$mzx,1);
			
			$shenshas_d[]=cls_paipan::Shen_rigan($rg,$rz);
			$shenshas_d[]=cls_paipan::Shen_rizhi($rz,$rz,$rzx);
			$shenshas_d[]=cls_paipan::Shen_yuezhi($mz,$rg,$rz);
			$shenshas_d[]=cls_paipan::Shen_nianzhi($yz,$rz,$rzx,1);
			
			$shenshas_h[]=cls_paipan::Shen_rigan($hg,$hz);
			$shenshas_h[]=cls_paipan::Shen_rizhi($rz,$hz,$hzx);
			$shenshas_h[]=cls_paipan::Shen_yuezhi($mz,$hg,$hz);
			$shenshas_h[]=cls_paipan::Shen_nianzhi($yz,$hz,$hzx,1);
		...
		$ykw=cls_paipan::xunkong($ygz);
		$mkw=cls_paipan::xunkong($mgz);
		$rkw=cls_paipan::xunkong($rgz);
		$hkw=cls_paipan::xunkong($hgz);
		
		
        $data_Array = array('taiyuan'=>$taiyuan,'nayintaiyuan'=>$nayintaiyuan,'minggong'=>$minggong,'zysj'=>$zysj,'sub_zysj'=>substr($zysj,3,1),'shishen'=>$shishen,'riyuan'=>$riyuan,'sizhu'=>$sizhu,'jqstr'=>$jqstr,'shishen1'=>$shishen1,'shishen2'=>$shishen2,'shishen3'=>$shishen3,'shishen4'=>$shishen4,'ygz'=>$ygz,'mgz'=>$mgz,'rgz'=>$rgz,'hgz'=>$hgz,'xkrgz'=>cls_paipan::xunkong($rgz),'nayinminggong'=>$nayinminggong,'zanggan1'=>$zanggan1,'zanggan2'=>$zanggan2,'zanggan3'=>$zanggan3,'zanggan4'=>$zanggan4,'nayin1'=>$nayin1,'nayin2'=>$nayin2,'nayin3'=>$nayin3,'nayin4'=>$nayin4,'jqstr'=>$jqstr,'ykw'=>$ykw,'mkw'=>$mkw,'rkw'=>$rkw,'hkw'=>$hkw,'srchangsheng'=>$srchangsheng,'shenshas_y'=>$shenshas_y,'shenshas_m'=>$shenshas_m,'shenshas_d'=>$shenshas_d,'shenshas_h'=>$shenshas_h);
        return $data_Array;
    }
