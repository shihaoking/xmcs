...
class mod_peidui { 
	...
	function hehun(){
		...
		*/
		...
		####男命
		$name=req::item("user"); #姓名
		$year=req::item("year"); #年干
		$month =req::item("month"); #月份
		$day =req::item("day"); #日
		$t_ime=req::item("t_ime"); #时
		$csrq=req::item("csrq");  #出生地址
		$qq=req::item("qq"); #
		$tel=req::item("tel"); #电话
		####女命
		$name_a=req::item("user_a");; #姓名
		$year_a=req::item("year_a");; #年干
		$month_a =req::item("month_a");; #月份
		$day_a =req::item("day_a");; #日
		$t_ime_a=req::item("t_ime_a");; #时
		...
		//if( !$name || !$name_a || !$year_a || !$year || !$month || !$month_a || !$day || !$day_a) { die("<script>alert('带*号的表单必须填写！');history.back();</script>"); }
		...
		for($i=1;$i<9;$i++) {
			...
			$shishen_for1 .=  '<td width="63">'.$a[$xy].'</td>';
		}						
		...
		$shishen_for2 .=  '<td width="63">'.$a[$xy_a].'</td>';
