                        ...
                        <td>
                            ...
                            <{if isset($_dbfield.editTableField) && !empty($_dbfield.editTableField)}>
                                <{if isset($_allowAction.edit) && !empty($_allowAction.edit)}>
                                    <a href="?ct=<{$ct}>&amp;ac=edit&amp;tpl_id=<{$v[$_dbfield.mainKey]}>&amp;page_no=<{$current_page}>" class="btn edit" style="cursor:pointer;"><{if $_allowAction.edit.title!=''}><{$_allowAction.edit.title}><{else}>修改<{/if}></a>
                                    &nbsp;&nbsp;
                                <{/if}>
                            <{/if}>

                            <{if isset($_allowAction.delete) && !empty($_allowAction.delete)}>
                                <a class="btn delete" jsbotton="confirm" isconfirm="1" href="?ct=<{$ct}>&amp;ac=delete&amp;tpl_id=<{$v[$_dbfield.mainKey]}>" title="注意：请谨慎操作。"><{if $_allowAction.delete.title!=''}><{$_allowAction.delete.title}><{else}>删<{/if}></a>
                            <{/if}>
        ...
        <button type="button" jstype="btn_post" isconfirm="1" url="?ct=<{$request.ct}>&ac=video_batch_del">批量删除</button>-->
            <{if isset($_allowAction.add) && !empty($_allowAction.add)}>
                <{if $_allowAction.add.type=='dialog'}>
                    <A href="?ct=<{$ct}>&ac=add&TB_iframe=true&width=<{$_allowAction.add.width+300}>&height=<{$_allowAction.add.height+300}>">添加</A>
                    <button type="button" onclick="tb_show('<{$_allowAction.add.title}>','?ct=<{$ct}>&ac=add&TB_iframe=true&width=<{$_allowAction.add.width+300}>&height=<{$_allowAction.add.height}>',true);"><{if $_allowAction.add.title!=''}><{$_allowAction.add.title}><{else}>新增<{/if}></button>&nbsp; &nbsp;
                <{else}>
