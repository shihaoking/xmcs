                    ...
                    <td>
                        ...
                        <{if isset($_dbfield.editTableField) && !empty($_dbfield.editTableField)}>
                            <{if isset($_allowAction.edit) && !empty($_allowAction.edit)}>
                                <a href="?ct=<{$ct}>&amp;ac=edit&amp;zt_id=<{$v[$_dbfield.mainKey]}>&amp;page_no=<{$current_page}>" class="btn edit" style="cursor:pointer;"><{if $_allowAction.edit.title!=''}><{$_allowAction.edit.title}><{else}>修改<{/if}></a>
                        &nbsp;&nbsp;
                            <{/if}>
                        <{/if}>

                        <{if isset($_allowAction.delete) && !empty($_allowAction.delete)}>
                            <a class="btn delete" jsbotton="confirm" isconfirm="1" href="?ct=<{$ct}>&amp;ac=delete&amp;zt_id=<{$v[$_dbfield.mainKey]}>" title="注意：请谨慎操作。"><{if $_allowAction.delete.title!=''}><{$_allowAction.delete.title}><{else}>删<{/if}></a>
                        <{/if}>
                        ...
                        <{if isset($_allowAction._extend) && !empty($_allowAction._extend)}>
                            <{foreach from=$_allowAction._extend item=vext key=url}>
                                ...
                                <{elseif $vext.type=='new'}>
                                    <{if $v.zt_type==2}>
                                        <a href='<{$v[$vext.paramfrom]}>' target="_blank" class="btn" style="cursor:pointer;"><{if $vext.title!=''}><{$vext.title}><{else}>未知操作名称<{/if}></a>
                                    <{else}>
                                        <a href='<{$url}>&<{$vext.paramto}>=<{$v[$vext.paramfrom]}>' target="_blank" class="btn" style="cursor:pointer;"><{if $vext.title!=''}><{$vext.title}><{else}>未知操作名称<{/if}></a>
                                    <{/if}>
                                    &nbsp;&nbsp;
                                    <{else}>
