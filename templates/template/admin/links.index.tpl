...
<div style="background-color:#DAFBCA;padding:5px 10px;line-height:32px;">
    ...
    <option value="<{$kk}>" <?php if($this->_tpl_vars['search'.$this->_tpl_vars['k']]==$this->_tpl_vars['kk'])echo 'selected';?> ><{$vv}></option>
        ...
        <button type="submit">搜索</button><span style="margin-left:25px;">&nbsp;&nbsp;&nbsp;<button onclick="tb_show('生成主链接','?ct=links&amp;ac=makeSite&amp;TB_iframe=true&amp;width=800&amp;height=700',true);" type="button">生成主链接</button>&nbsp;&nbsp;&nbsp;<button onclick="tb_show('生成友链','?ct=links&amp;ac=makerightlinks&amp;TB_iframe=true&amp;width=800&amp;height=700',true);" type="button">生成友链</button>
            <?php } ?>
...
<form id="form_list" name="form_list" method="POST" action="?ct=<{$ct}>&amp;ac=batch_update">
    ...
    <table class="table-sort">
        ...
        <{if !empty($data_list)}>
            ...
            <{else}>
            <tr>
                <td></td><td colspan="<{$_dbfield.allTableField|@count}>" align="center">数据为空</td>
            </tr>
            <{/if}>
