...
<{if isset($_allowAction.add) && !empty($_allowAction.add)}>
                <{if $_allowAction.add.type=='dialog'}>
                    <button type="button" style="margin: 10px 24px;" onclick="tb_show('<{$_allowAction.add.title}>','?ct=<{$ct}>&ac=add&TB_iframe=true&width=<{$_allowAction.add.width}>&height=<{$_allowAction.add.height}>',true);"><{if $_allowAction.add.title!=''}>我要提现<{else}>我要提现<{/if}></button>&nbsp; &nbsp;
                <{else}>
                    <a href="?ct=<{$ct}>&amp;ac=add" class="btn add" style="cursor:pointer;"><{if $_allowAction.add.title!=''}>我要提现<{else}>新增<{/if}></a>
                    &nbsp;&nbsp;
                <{/if}>
            <{/if}>
                    ...
                    <td>
                        ...
                        <{if isset($_dbfield.editTableField) && !empty($_dbfield.editTableField)}>
                            <{if isset($_allowAction.edit) && !empty($_allowAction.edit)}>
                                <a href="?ct=<{$ct}>&amp;ac=edit&amp;id=<{$v[$_dbfield.mainKey]}>&amp;page_no=<{$current_page}>" class="btn edit" style="cursor:pointer;"><{if $_allowAction.edit.title!=''}>打款<{else}>修改<{/if}></a>
                        &nbsp;&nbsp;
                            <{/if}>
                        <{/if}>
...
<div id="bottom">
    <form method="POST" action="" bind=".table-sort">
        ...
        <button type="button" jstype="btn_post" isconfirm="1" url="?ct=<{$request.ct}>&ac=video_batch_del">批量删除</button>-->
            <{if isset($_allowAction.add) && !empty($_allowAction.add)}>
                <{if $_allowAction.add.type=='dialog'}>
                    <button type="button" onclick="tb_show('<{$_allowAction.add.title}>','?ct=<{$ct}>&ac=add&TB_iframe=true&width=<{$_allowAction.add.width}>&height=<{$_allowAction.add.height}>',true);"><{if $_allowAction.add.title!=''}>我要提现<{else}>我要提现<{/if}></button>&nbsp; &nbsp;
                <{else}>
                    <a href="?ct=<{$ct}>&amp;ac=add" class="btn add" style="cursor:pointer;"><{if $_allowAction.add.title!=''}>我要提现<{else}>新增<{/if}></a>
                    &nbsp;&nbsp;
                <{/if}>
