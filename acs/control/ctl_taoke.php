...
class ctl_taoke extends common{
    // 自动验证设置
    protected $_submit_validate     =   array(
        //'content'=>array('','notempty','标题必须！','all'),
    ...
    public function __construct()
    {
		...
		$sql = 'select * from `sm_class` where parentid="417"';
...
//commentCount, favoriteCount, STATUS, intdate, updatetime, addtime
        $this->_dbfield=array(
            'mainKey'=>'id',
            'allTableField'=>array('id'=>'编号','tid'=>'栏目','content'=>'内容'),
            'addTableField'=>array('title','','content'),
			
            'editTableField'=>array('id','tid','content'),
           
            'listTableField'=>array('id','tid','content'),
			
            'batchUpdateTableField'=>array('tid','id'),
