...
class ctl_xingming extends common{
    // 自动验证设置
    protected $_submit_validate     =   array(
       // 'xid'=>array('','notempty','姓氏一定要填写！','all'),
...
//        $this->cps = array('duowan'=>'多玩','juyouqu'=>'juyouqu','laifudao'=>'来福岛','3jy'=>'3jy','baozoumanhua'=>'暴走漫画','mahua'=>'mahua','youku'=>'youku','haha'=>'haha','negui'=>'乐归网','jokeji'=>'jokeji','wasu'=>'华数','zol'=>'zol', 'duzhebao'=>'读者宝');
		...
		foreach($page_list as $k=>$v){
			$categorys_arr[$v['id']] = $v['xing'];
		}
		
        $this->categorys = $categorys_arr;
		$this->sex = array(0=>'不限',1=>'男',2=>'女');
...
//commentCount, favoriteCount, STATUS, intdate, updatetime, addtime
        $this->_dbfield=array(
            'mainKey'=>'id',
            'allTableField'=>array('name'=>'姓名','sex'=>'性别','geshu'=>'个数'),
            'addTableField'=>array('name','sex','geshu'),
            'editTableField'=>array('name','sex','geshu'),
           
            'listTableField'=>array('name','sex','geshu'),
			
            'batchUpdateTableField'=>array('name','sex'),
			...
			'sex'=>array(
				'element'=>array('e_name'=>'select','datafrom'=>$this->sex),
                'search'=>'1',
            ),
			'geshu'=>array(
				'element'=>array('e_name'=>'input','e_type'=>'text'),
