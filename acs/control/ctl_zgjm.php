...
class ctl_zgjm extends common{
    ...
    public function __construct()
    {
		...
		$sql = 'select * from `sm_class` where parentid="349"';
...
//commentCount, favoriteCount, STATUS, intdate, updatetime, addtime
        $this->_dbfield=array(
            'mainKey'=>'id',
            'allTableField'=>array('id'=>'编号','title'=>'标题','tid'=>'栏目','status'=>'状态','order'=>'排序(越大排越前)','uptime'=>'更新时间','content'=>'内容','img'=>'图片','shorttxt'=>'描述'),
            'addTableField'=>array('title','tid','content','shorttxt'),
			
            'editTableField'=>array('title','tid','status','content','uptime','order','shorttxt'),
           
            'listTableField'=>array('id','title','tid','status','uptime','order','shorttxt'),
			
            'batchUpdateTableField'=>array('title','tid','status','uptime','order'),
    ...
    public function updatestatus(){
        echo 'start<br />';
        if(db::query('update zgjm_data set `status`="1" where `status`="99" limit 1000')){
            echo 'success!';
        }else{
            echo 'fail!';
        }
